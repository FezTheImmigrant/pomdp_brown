...
class GridComponent:
    ...
    def initializeGridWorld(self):
        self.Utility = [[ 0 for c in range(self.Parameters.Col) ] for r in range(self.Parameters.Row)]
        self.Policy = [[ "<" for c in range(self.Parameters.Col) ] for r in range(self.Parameters.Row)]
        ...
        for end_state in self.Parameters.EndStates:
            if end_state[0] < self.Parameters.Row and end_state[1] < self.Parameters.Col:
                if self.Utility[end_state[0]][end_state[1]] == 'X':
                    print ("Given end state cannot be placed where wall currently is.", end_state)
                    return False
